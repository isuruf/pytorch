...
from ...utils._sympy.symbol import free_symbol_is_type, PrefixedSymbol, prefix_str, symbol_is_type, SymT
...
block_offsets = {
    symt: PrefixedSymbol(f"{prefix_str[symt]}offset", prefix=symt, integer=True, nonnegative=True)
    for symt in [SymT.XBLOCK, SymT.YBLOCK, SymT.RINDEX]
}

block_sizes = {
    symt: PrefixedSymbol(f"{prefix_str[symt].upper()}BLOCK", prefix=symt, integer=True, positive=True)
    for symt in [SymT.XBLOCK, SymT.YBLOCK, SymT.RINDEX]
}
